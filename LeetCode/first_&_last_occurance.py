...
def firstAndLastPosition(arr, n, k):
    ...
    while l <= r:
        ...
        if arr[mid] == k:
            first_occurance = mid
            # eventhough the element is found, we traverse through the left array to find the first occurance and store it instead.
            r = mid - 1
        if arr[mid] > k:
            r = mid - 1
        if arr[mid] < k:
    ...
    while l <= r:
        ...
        if arr[mid] == k:
            last_occurance = mid
            # eventhough the element is found, we traverse through the right array to find the last occurance and store it instead.
            l = mid + 1
        if arr[mid] > k:
            r = mid - 1
        if arr[mid] < k:
